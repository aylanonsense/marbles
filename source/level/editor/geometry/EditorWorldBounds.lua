...
class("EditorWorldBounds").extends("EditorPolygon")

function EditorWorldBounds:init(points)
	EditorWorldBounds.super.init(self, EditorGeometry.Type.Polygon)
	self.points = points
...
function EditorWorldBounds:draw()
	local coordinates = {}
...
function EditorWorldBounds:getEditTargets()
...
function EditorWorldBounds:getMidPoint()
	local avgX, avgY = 0, 0
...
function EditorWorldBounds:getTranslationPoint()
	return self.points[1].x, self.points[1].y
end

function EditorWorldBounds:translate(x, y)
	for _, point in ipairs(self.points) do
		point:translate(x, y)
	end
end

function EditorWorldBounds:delete()
	return removeItem(scene.geometry, self)
end

function EditorWorldBounds:isClockwise()
	local sum = 0
